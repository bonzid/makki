...
class RequestHandler {
    ...
    function __construct(string $url, string $server, string $headers) {
        ...
        //die();
        //erreur affichée sur page d'accueil : array(1) { [0]=> string(1) "/" }
        $this->uri              = $url;
        ...
        if ($this->method == 'GET') {
            ...
            if (isset($this->request['collection']) && in_array($this->request['collection'], self::RESOURCES_COLL)) {
                ...
                else if (isset($this->request['collection']) && $this->request['collection'] == "remerciements") {
                    $page = "remerciements";
                }
                ...
                else if (isset($this->request['collection']) && $this->request['collection'] == "bibliographie") {
                    $page = "bibliographie";
                }


                //PAGE CONNAISSEZ VOUS CE MOT
                else if (isset($this->request['collection']) && $this->request['collection'] == "quiconnait") {
                    $query = array("quic" => array("true"));

                    if (isset($this->request['target'])) {
                        $page = "quiconnait";
                        $word_data = $this->get_data(array("title" => $this->request['query'], "quic" => array("true")), $raw);
                        ...
                        //TODO: Afficher une liste de tous les mots dans l'ordre alphabétique
                        $page = "quiconnait";
                        $word_data = $this->get_data(array("quic" => array("true")), $raw);
                    }
                }
                    ...
                    //var_dump($this->request['collection']); // Debug
                    //var_dump("ici");
                    //die();
                    $this->redirect("", "badreq_err", 400);
                    return;
     ...
     */
    function search_query(array $query): array { //prend en entrée tableau associatif $query et retourne un tableau
        //var_dump($body);
        //var_dump($query);
        //exit;

        //correspondance des critères de recherche avec les termes du vocabulaire RDF
        $corres = array(
        ...
        //initialisation de la clause WHERE (requête SparQL)
        //définit que chaque élément (?in) possède un titre
        $body = "WHERE { 
            ?in dcterms:title ?title . 
            ?in dcterms:abstract ?abstract .
        ";
        

        //boucle sur chaque critère de recherche $criterion

        //--------------AJOUT--------------
        foreach(array_keys($query) as $criterion) {
            //on vérifie ici la validité du critère
            if (!array_key_exists($criterion, $corres) || !is_array($query[$criterion]) || count($query[$criterion]) <= 0) {
                continue;
            }

            //--------------AJOUT--------------
            // Gestion spécifique de "quic" == "true"
            if ($criterion === "quic" && in_array("true", $query[$criterion])) {
                $body .= " ?in dcterms:quic \"true\" . ";
                continue;
            }
            //--------------FIN AJOUT--------------
        ...
        //gestion de la pagination (nb de pages etc)
        ...
        //création de la requête SparQL finale
        $q = "DESCRIBE ?in $body ORDER BY (?title) LIMIT $page_size OFFSET ".$offset;

        //retourne les résultats avec : 
        //  - les infos de pagination
        //  - les données obtenues par la requête SparQL
     ...
     */
     ...
     //définition de la fonction : prend en param $query (tableau contenant requête utilisateur gérée par RequestParser), $raw (booléen qui retourne les resultats bruts sans mise en forme).
     //elle retourne un tabelau contenant les données formatées
    function get_data(array $query, bool $raw = false): array {

        // On prépare puis effectue la requête pour récupérer les infos :
        $retrieved = $this->search_query($query); //On appelle search_query($query) pour récupérer les résultats de la recherche.
        //var_dump($retrieved); // Vérifie les résultats récupérés

        $res = $retrieved["data"]; //contient les résultats bruts

        // Si aucune donnée n'est récupérée, retourne un tableau vide
        if (empty($res['result'])) {
            return array("pagination" => $retrieved["pagination"], "data" => []);
        }


        //var_dump($res);  // Ajoute cette ligne pour voir les données récupérées


        if ($raw) { //si $raw==true, on retourne les résultats sans transformation
            return $res['result'];
        }
        ...
        //parcours des résultats : boucle sur chaque entrée dans résultats bruts
        foreach(array_keys($res['result']) as $def_id) {
            $def = array(); //stocke ls informations de l'entrée courante
            $words = array(); //stocke les titres
            $syns = array(); //stocke les synonymes

            //parcours des prédicats pour l'entrée courante
            foreach(array_keys($res['result'][$def_id]) as $pred) { 
                $item = substr($pred, strrpos($pred, "/")+1); //extraction du nom du prédicat (titre, subject...)

                if ($item == 'title') { //si le prédicat est titre, on stocke la valeur dans $words
                    foreach($res['result'][$def_id][$pred] as $element) {
                        array_push($words, $element['value']);
                    }
                }
                else if ($item == 'identifier') { //si c'est un identifier,
                    
                    //on crée une requête pour recup les synonymes
                    ...
                    //on récupère les synonymes par l'exécution de la requête
                    $res_syn = $this->db->query($q_s);

                    if (sizeof($res_syn['result']['rows']) > 0) {
                        //stockage des synonymes
                        //si syn n'existe pas encore dans $def, on l'initialise
                        if (!array_key_exists("syn", $def)) {
                            $def["syn"] = array();
                        }
    
                        foreach($res_syn['result']['rows'] as $element) {
                            //value : titre du synonyme
                            //lang : langue par défaut ou ar si absente
                            //URI identifiant de l'entrée correspondante
                ...
                //ici traitement des autres prédicats
                //si le prédicat n'est ni titre, ni identifier, on le stocke normalement
                else {
                    ...
                    foreach($res['result'][$def_id][$pred] as $element) {
                        //stocké sous forme d'un tableau associatif avec la valeur et la langue par défaut
                        array_push($def[$item], ["value" => $element['value'], "lang" => isset($element['lang']) ? $element['lang'] : "fr"]);
                    }
                }
            }

            //organisation des résultats dans output
            //boucle sur chaque résultat en associant titre à def_id
            foreach($words as $w) {
        ...
        //var_dump($res['result']); // Vérifie les résultats après le filtrage
        //var_dump($output); // Vérifie la structure des résultats formatés

        //var_dump($word_data['data']);
        //var_dump($query); // Vérifie les paramètres envoyés à la requête


        //retour des résultats formatées ($output) avec les infos de pagination de search_query() 
