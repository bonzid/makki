...
class FileUploader {
    ...
    function __construct(array $valid_ext, int $user_id, int $max_size=500000) {
        ...
        if(is_dir($this->uploads_path)){
            ...
            if(!is_writable($this->uploads_path)){
                ...
                logToConsole("<br/>Upload dir is not writeable. Aborting.<br/>");
                http_response_code(500);
            }
        }else{
            if(!mkdir($this->uploads_path, 0777)){
                print "<br/>Couldn't make the directory for upload : '".$this->uploads_path."'. Aborting.<br/>";
                logToConsole("<br/>Couldn't make the directory for upload : '".$this->uploads_path."'. Aborting.<br/>");
                http_response_code(500);
            }
            else {
    ...
    public function upload(array $filelist): bool {
        ...
        if(!isset($filelist)){
            ...
            logToConsole("<br/>No files to upload. Aborting.<br/>");
            return false;
        }
        ...
        for ($i = 0 ; $i < sizeof($filelist["size"]) ; $i++) {
            ...
            if (!in_array($fname, $this->valid_ext)) {
                ...
                logToConsole("<br/>Wrong filetype. Aborting.<br/>");
                return false;
            }
        }
        ...
        if($size > $this->max_size){
            ...
            logToConsole("<br/>Upload size is too big : ".$size.". Aborting.<br/>");
            return false;
        }
        ...
        for ($i = 0 ; $i < sizeof($filelist["size"]) ; $i++) {
            ...
            if (move_uploaded_file($filelist['tmp_name'][$i], $target_path)) {
                ...
                if (!chmod($target_path, 0666)) {
                    ...
                    logToConsole("<br/>Something went wrong with permissions...<br/>");
                    return false;
                }
            } else {
                echo "<br/>Possible file upload attack! File '".$filelist['name'][$i]."' not uploaded at $target_path.<br/>";
                logToConsole("<br/>Possible file upload attack! File '".$filelist['name'][$i]."' not uploaded at $target_path.<br/>");
                return false;
            }
        }
    ...
    public function delete() {
        ...
        foreach($this->files as $del_file){
            ...
            if(!is_file($del_file)){
                ...
                logToConsole("<br/>Something went wrong while deleting $del_file<br/>");
            }

            if(!unlink($del_file)){
                echo "<br/>Couldn't delete file '".$del_file."'!<br/>";
                logToConsole("<br/>Couldn't delete file '".$del_file."'!<br/>");
            }
        }
    }
}
